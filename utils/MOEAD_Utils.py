...
import numpy as np
from utils.Mean_Vector_Util import Mean_vector
...
def Load_W(moead):
    ...
    if os.path.exists(path) == False:
        print('not exists')
        mv = Mean_vector(moead.Test_fun.Dimention, moead.Test_fun.Func_num, path)
        mv.generate()
        print('created')
...
def is_dominate(moead, F_X, F_Y):
    ...
    i = 0
    ###自定义的
    for index, problem_type in enumerate(moead.Z):
        if problem_type == 0:  # 最小化问题
            if F_X[index] < F_Y[index]:
                i = i + 1
            else:
                return False
        elif problem_type == 1:  # 最大化问题
            if F_X[index] > F_Y[index]:
                i = i + 1
            else:
                return False
...
def cpt_tchbycheff(moead, idx, X):
    ...
    print(X)
    print(type(X))
    # F_X = moead.Test_fun.Func(X, moead.XX.XX, moead.XX.y, moead.XX.method, moead.XX.n_clfs, moead.XX.fs_functions,
    #                           moead.XX.score_name)
    F_X = moead.Test_fun.Func(X, moead.XX.XX, moead.XX.y, moead.XX.method, moead.XX.n_clfs, moead.XX.fs_functions,
                              moead.XX.score_name)

    print("MOEAD___Utils",F_X)
...
def update_Z(moead, Y):
    ...
    dz = np.random.rand()
    # F_y = moead.Test_fun.Func(Y)
    F_y = moead.Test_fun.Func(Y, moead.XX.XX, moead.XX.y, moead.XX.method, moead.XX.n_clfs, moead.XX.fs_functions,
                              moead.XX.score_name)
